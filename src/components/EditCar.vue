          ...
          <h2 class="mt-3">Add Car</h2>
            <div class="mt-3">
              <div class="row">
                ...
                <button type="submit" class="btn btn-primary mt-3" @click.prevent="updateCar()">Update</button>
            </div>
        </div>
    </div>
</template>
<script>
  ...
  export default {
    ...
    editCar(){
        ...
        this.$store.dispatch('car/getCar', id)
      .then((response) =>{
    ...
    updateCar(){
      ...
      if(!this.v$.$error){
        this.$store.dispatch('car/updateCar',this.car)
        .then(() => {
          this.$router.push('/');
        })
      }
